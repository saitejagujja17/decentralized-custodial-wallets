...
contract AdvancedTimeVault2 {
    ...
    function depositToken(address token, uint amount, string calldata label, uint256 unlockAfterSeconds, address beneficiary) external {
    ...
    function withdraw(uint vaultId, uint withdrawAmount) external {
        ...
        if (v.token == address(0)) {
            payable(v.beneficiary).transfer(withdrawAmount);
        } else {
            require(IERC20(v.token).transfer(v.beneficiary, withdrawAmount), "Token transfer failed");
        }
