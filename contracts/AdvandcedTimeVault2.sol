...
contract AdvancedTimeVault2 {
    ...
    // ETH deposit with label and unlock time (in seconds from now)
    function depositETH(string calldata label, uint256 unlockAfterSeconds) external payable {
        ...
        emit Deposited(msg.sender, userVaults[msg.sender].length - 1, address(0), msg.value, unlockAt, label);
    }

    // ERC20 deposit with label and unlock time
    function depositToken(address token, uint amount, string calldata label, uint256 unlockAfterSeconds) external {
        ...
        emit Deposited(msg.sender, userVaults[msg.sender].length - 1, token, amount, unlockAt, label);
    }
...
function withdraw(uint vaultId, uint withdrawAmount) external {
    ...
    if (v.amount == 0) v.withdrawn = true;
